...
4	1    4    8    5"""
    ...
    def test_task_1(self):
        self.assertEqual(Polyedr(f"data/ccc.geom").task(), 0)

    # Есть невидимые рёбра, но их центр не попадает в нужную область
    def test_task_2(self):
        self.assertEqual(Polyedr(f"data/big_cube.geom").task(), 0)

    # Есть невидимые рёбра, попадающие в окружность с R = 2
    def test_task_3(self):
        self.assertAlmostEqual(Polyedr(f"data/cube.geom").task(), 3)
